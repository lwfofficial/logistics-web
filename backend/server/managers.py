...
from core.models import OrderFeedback
from mobile.models import InstantDeliveryOrderFeedback


class FeedbackManager(object):

    @staticmethod
    def updateBuyerProfileFeedback(profile):
        orderFeeds = OrderFeedback.objects \
            .filter(order__profile=profile) \
            .exclude(profile=profile).distinct()
        orderFeeds = map(lambda f: f.score, orderFeeds)
        instantOrderFeeds = InstantDeliveryOrderFeedback.objects \
            .filter(order__buyer=profile) \
            .exclude(profile=profile).distinct()
        instantOrderFeeds = map(lambda f: f.score, instantOrderFeeds)
        feeds = orderFeeds + instantOrderFeeds
        return functools.reduce(lambda x, y: x + y, feeds) / len(feeds)

    @staticmethod
    def updateForwarderProfileFeedback(profile):
        orderFeeds = OrderFeedback.objects \
            .filter(order__service__profile=profile) \
            .exclude(profile=profile).distinct()
        orderFeeds = map(lambda f: f.score, orderFeeds)
        instantOrderFeeds = InstantDeliveryOrderFeedback.objects \
            .filter(order__courier=profile) \
            .exclude(profile=profile).distinct()
        instantOrderFeeds = map(lambda f: f.score, instantOrderFeeds)
        feeds = orderFeeds + instantOrderFeeds
        return functools.reduce(lambda x, y: x + y, feeds) / len(feeds)
...
class CountryManager(object):
    ...
    @staticmethod
    def countryByCoutryCode(countryCode):
        country = filter(lambda c: c['code'] == countryCode, CountryManager.COUNTRY_CODES)
